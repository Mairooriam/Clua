...
static void check_sqlite_result(int rc, sqlite3* db, const char* context) {
...
static void init_test_data_writer(sqlite3* db) {
    const char* sql = R"sql(
    ...
    check_sqlite_result(rc, db, "prepare data insert");
}
...
int main(int, char**) {
    ...
    while (!done) {
        ...
        while (!done) {
            ...
            frame_counter++;
            const sqlite3_int64 timestamp_ms =
            ...
            if (frame_counter % 300 == 0) {
                insert_test_value(
                    db, timestamp_ms, 3, 50.0 + std::sin(frame_counter * 0.008) * 10.0);
            }
            ...
            // application based on those two flags. [If using SDL_MAIN_USE_CALLBACKS: call
