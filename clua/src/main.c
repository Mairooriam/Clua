...
static void handler_TheAnswerChanged(
    ...
    UA_DataValue* value) {
    if (value->hasValue) {
        log_trace("callback from %i", subId);
        mtx_lock(&cache_mutex);
        if (cache_count < CACHE_SIZE) {
            boolean is = UA_Variant_isScalar(&value->value);
            is ? log_trace("Hello from callback value is scalar") : log_trace("Hello no scalar");
            cache[cache_count].nodeId =
                UA_NODEID_STRING_ALLOC(5, "::AsGlobalPV:Basetim");  // Use actual nodeid
        ...
        for (int i = 0; i < cache_count; ++i) {
            ...
            if (UA_Variant_isScalar(&entryData->value)) {
                ...
                if (type == &UA_TYPES[UA_TYPES_BOOLEAN]) {
                    snprintf(valStr, 63, "%i", *(int*)entryData->value.data);
                } else if (type == &UA_TYPES[UA_TYPES_DOUBLE]) {
                    snprintf(valStr, 63, "%f", *(double*)entryData->value.data);
                ...
                snprintf(
                    sql,
                    sizeof(sql),
                    "INSERT INTO plc_data (nodeid, value, timestamp, status) VALUES ('%s', %f, "
                    "'%s', '%u');",
                    "test",
                    val,
                    timestamp,
...
#define COMMAND_COUNT (sizeof(commands) / sizeof(commands[0]))
    ...
    void cmd_config_edit(MiruaContext * ctx, const char* args) {
        ...
        if (sscanf(args, "%d %127[^\n]", &index, value) != 2) {
            log_error(
                "Invalid input. { %s }", get_command("edit", mirua_state_get(ctx))->description);
        }
